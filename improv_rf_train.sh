...
for d in $dataset_folders; do
    ...
    python improve_rf.py ../dataset/synthetic/${d}/train_${d}.sample ../dataset/synthetic/${d}/test_${d}.sample
    echo ++++ train complete!: $d
    # copy results folder
    mkdir -p ../02-output/improve_rf/$d
    cp -r results ../02-output/improve_rf/$d/
